...
int evaluate_window(vector<int> window,int piece){
    ...
    else if(countPiece == 3 and countEmpty == 1)score +=5;
    else if(countPiece == 2 and countEmpty == 2)score +=2;

    int countOppPiece = 0;
    for(int i =0;i<window.size();i++){
        if(window[i] == opp_piece)countOppPiece++;
    }

    if(countOppPiece == 3 and countEmpty == 1)score-=4;
...
int scorePosition(vector<vector<int>> board,int piece){
    ...
    for(int i =0;i<center_array.size();i++){
        if(center_array[i] == piece)center_count++;
    }
...
vector<int> minimax(vector<vector<int>> board,int depth,int alpha,int beta,int maximizingPlayer ){
     ...
     if(depth==0 or isTerminal){
        if(isTerminal){
            ...
            if(isGameOver(board,Ai))return v1;
            else if(isGameOver(board,Player))return v2;
            else return v3;
         }
        else {
            vector<int> temp;
